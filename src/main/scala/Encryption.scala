...
/**
 * Created by ramit on 12/15/2015.
 */

/*case class Encrypt(dataToEncrypt: String, key: String)
case class Decrypt(dataToDecrypt: String, key: String)

class Encryption extends Actor{
    ...
    def receive = {
      case Encrypt(dataToEncrypt, key) => {
         sender() ! encrypt(key, dataToEncrypt)
      }

      case Decrypt(dataToDecrypt, key) => {
        sender() ! decrypt(key, dataToDecrypt)
      }
